...
# Create more dense c values for smoother curves
...
for c in c_values:
    ...
    K1_equilibrium.append(solution[-1, 0])
    K2_equilibrium.append(solution[-1, 1])
    L1_equilibrium.append(solution[-1, 2])
    L2_equilibrium.append(solution[-1, 3])

    # Store total values
    K_total.append(solution[-1, 0] + solution[-1, 1])
    L_total.append(solution[-1, 2] + solution[-1, 3])

# Create subplots
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

# Plot labor equilibrium (Figure 5.3)
ax1.plot(c_values, L1_equilibrium, 'b-', label='L1', linewidth=2)
ax1.plot(c_values, L2_equilibrium, 'r--', label='L2', linewidth=2)
ax1.plot(c_values, L_total, 'g-', label='Total Labor', linewidth=2)
ax1.set_xlabel('c')
ax1.set_ylabel('Labor')
ax1.legend()
ax1.grid(True, linestyle='--', alpha=0.7)

# Plot capital equilibrium (Figure 5.4)
ax2.plot(c_values, K1_equilibrium, 'b-', label='K1', linewidth=2)
ax2.plot(c_values, K2_equilibrium, 'r--', label='K2', linewidth=2)
ax2.plot(c_values, K_total, 'g-', label='Total Capital', linewidth=2)
ax2.set_xlabel('c')
